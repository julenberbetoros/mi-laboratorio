...
# Soluciones de los ejercicios 01-10 con solver 10 que intenta reconstruir
# completamente la clave (crib + búsqueda exhaustiva para pocos bytes desconocidos).
...
# ======================================================
# === EJERCICIO 10: Crib + búsqueda de bytes desconocidos ==
# ======================================================
...
def fill_and_test_key(cipher: bytes, key_partial: List[Optional[int]], max_unknowns_bruteforce: int = 3) -> Optional[str]:
    """
    Si el número de posiciones None en key_partial <= max_unknowns_bruteforce,
    fuerza bruta esas posiciones y prueba cada key completa. Devuelve la flag si la encuentra.
    """
...
def solve_10_secret_key_xor() -> str:
    ...
    n = len(cipher)
    crib = b"crypto{"
    # intentar keylen plausibles; por tus salidas anteriores 8 es probable, pero probamos 1..40
    for keylen in range(1, 41):
        for offset in range(0, n - len(crib) + 1):
            ...
            res = fill_and_test_key(cipher, key_partial, max_unknowns_bruteforce=3)
            if res:
                return res
    # si no encontró nada, devolvemos una sugerencia: derivar claves parciales para keylen=8 y mostrar top
    # (útil para depuración manual)
    # construiremos y mostraremos las claves parciales para keylen=8
    debug_lines = []
    keylen = 8
    for offset in range(0, n - len(crib) + 1):
...
def main(selected: Optional[int] = None) -> None:
    ...
    if selected is None:
        ...
        for k, (name, fn) in sorted(solvers.items()):
            ...
            print(result)
        print("\nPara ejecutar solo uno: python cryptohack_all.py 10")
    else:
